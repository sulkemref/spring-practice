...
public interface AccountRepository extends JpaRepository<Account, Long> {
    ...
    List<Account> findByAddressStartingWith(String addressKeyword);
    ...
    List<Account> retrieveAllAccountsSortedByAge();
    ...
    List<Account> retrieveAccountsByAgeLowerThan(int age);

    //Write a native query to read all accounts that a specific value can be containable in the name, address, country, state city
    @Query(value = "SELECT * FROM account_details  WHERE name = :name AND address = :address AND country = :country AND city = :city",nativeQuery = true)
    List<Account> retrieveAccountsByNameAndAddressAndCountryAndStateAndCity(@Param("name")String name,@Param("address") String address,@Param("country") String country,@Param("city") String city);
    ...
    @Query(value = "SELECT * FROM account_details  WHERE age > ?1",nativeQuery = true)
    List<Account> retrieveAccountsWhereAgeGraterThan(int age);

    @Query(value = "SELECT * FROM account_details  WHERE age > :age",nativeQuery = true)
    List<Account> retrieveAccountsWhereAgeParamGraterThan(@Param("age") int age);
